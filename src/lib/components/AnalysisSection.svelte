<script lang="ts">
  ...
  // Dùng computed state để lấy handle hiện tại
  // Dùng analysisData.profile.screen_name nếu đã có, nếu không thì dùng từ URL
  $: handle = analysisData?.profile?.screen_name || $page.url.searchParams.get('handle') || '';
  ...
  async function fetchProData(handle: string) {
      if (!handle) return; 
      // Bỏ qua nếu đã load, đang load, hoặc không phải user Pro
  ...
  function handleTabClick(tab: TabId) {
    ...
    if (user?.isPro && (tab === 'fixes-growth' || tab === 'monetization-kit')) {
        // Chỉ gọi fetchProData khi chuyển tab (nếu chưa load)
        fetchProData(handle);
    }
  }
  ...
  {#if isLoading || isProDataLoading}
    <section class="mt-0 max-w-4xl mx-auto text-center">
      ...
      >
        ...
        >
          <svg class="h-5 w-5 animate-spin text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 
24 24">
        ...
        <p class="mt-4 text-lg font-semibold text-gray-900">
          {#if isProDataLoading}Generating Pro Content...{:else}Analyzing profile...{/if} 
        </p>
        
        <p class="mt-1 text-sm text-gray-600 max-w-md mx-auto">
          {#if isProDataLoading}Please wait, our AI is writing your drafts & roadmap...{:else}Please wait, we're auditing the bio, pinned tweet, and last 20 posts...{/if}
        </p>
      </div>
    </section>
...
9 9 0 0 1 18 0Z"
        ...
        <p class="mt-1 text-sm text-gray-600 max-w-md mx-auto">
          Paste an X handle above and 
          hit
          <span class="font-medium">'Analyze profile'</span> to generate your personalized report.
        </p>
      </div>
    {/if}
                  ...
                  <div class="space-y-6">
                    ...
                    <div>
                      <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-primary 
text-white">
                      ...
                      <div class="mt-4 space-y-3 text-xs">
                        ...
                        <div class="w-32 text-gray-600">Offer Clarity</div>
                          <div class="flex-1">
                            <div class="h-2 rounded-full bg-gray-200">
                              <div 
class="h-2 rounded-full bg-primary" style="width: {analysisData.analysis.keyScores.offerClarity}%"></div>
                            </div>
                          </div>
            ...
            <div class="tab-content p-6 sm:p-8">
              ...
              >
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-500">
                    Optimized bio, pinned tweet drafts, and a deeper look at what actually
                    performs.
                  </p>
                </div>
                {#if !user?.isPro}
              ...
              {#if user?.isPro}
                ...
                <div class="space-y-8">
                  ...
                  <div class="grid gap-6 md:grid-cols-2">
                     <div class="border border-indigo-200 rounded-lg p-4 bg-indigo-50/70">
                      <p class="text-sm font-semibold text-gray-800">
                      ...
                      <p class="mt-2 text-base text-gray-900 font-bold">
                        ...
                        <span class="text-indigo-600 font-normal text-sm">
                          (Found in your {displayFixesData.highestImpactCount} highest-impact posts)
                        </span>
                      </p>
                    </div>
                    ...
                    <div class="border border-green-200 rounded-lg p-4 bg-green-50/70">
                      ...
                      <p class="mt-2 text-base text-gray-900 font-bold">
                        ...
                        <span class="text-green-600 font-normal text-sm">
                          ({displayFixesData.formatPercentage}% of your top posts use this)
                        </span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    <h3 class="text-base font-semibold text-gray-900 mb-3">3 Optimized Bio Drafts (Ready to Copy)</h3>
                    <div class="space-y-3">
                      {#each displayFixesData.bioDrafts as draft}
                        <div class="p-3 border border-gray-200 rounded-lg bg-white flex items-center justify-between">
                            <div class="flex-1 mr-4">
                                <p class="font-bold text-primary">{draft.title}</p>
                                <p class="mt-1 text-sm text-gray-700">{draft.content}</p>
                            </div>
                            <button
                                on:click={() => { navigator.clipboard.writeText(draft.content); addToast('Bio Copied!', 'success'); }}
                  ...
                  <div>
                    ...
                    <div class="p-4 border border-gray-200 rounded-xl bg-white shadow-soft">
                        <p class="font-bold text-primary mb-2">The "Conversion Funnel" Tweet</p>
                        <p class="whitespace-pre-wrap text-sm text-gray-800">
                            {displayFixesData.pinnedTweetCopy}
                        </p>
                        <button
                            on:click={() => { navigator.clipboard.writeText(displayFixesData.pinnedTweetCopy); addToast('Tweet Copied!', 'success'); }}
                  ...
                  <div>
                    <h3 class="text-base font-semibold text-gray-900 mb-3">Your Next 5 Content Ideas (Never get writer's block)</h3>
                    <ul class="space-y-2 text-sm text-gray-700 list-disc pl-5">
                      {#each displayFixesData.nextTweetIdeas as idea}
                        <li class="pl-1 text-gray-800 font-medium">{idea}</li>
                      {/each}
                    </ul>
                  </div>
                </div>
              ...
              {:else}
                <div class="relative">
                  ...
                  <div class="relative">
                    <div class="text-sm text-gray-400 space-y-4 blur-sm pointer-events-none">
                      <div class="p-3 border border-gray-200 rounded-lg bg-white">
                        <p class="font-bold text-primary">Bio Draft 1 (The Authority)</p>
                        <p>
                          Building @SaaSToolName from $0 to $10k MRR. Follow along for
                          transparent growth logs &amp; playbooks. I help indie devs launch
                          profitable products.
                        </p>
                      </div>
                      <div class="p-3 border border-gray-200 rounded-lg bg-white">
                        ...
                        <p>
                          Short-form threads (5-7 tweets) focused on a single tactical problem.
                          <strong>82% of your top posts</strong> use this format.
                        </p>
                      </div>
                      <div class="p-3 border border-gray-200 rounded-lg bg-white">
                        <p class="font-bold text-primary">Your Pinned Tweet Copy:</p>
                        <p class="italic">
                          "If you're a founder struggling with initial traction, this thread is
                          for you. I detail the 3 systems I use to get my first 100 paid
                          users..."
                        </p>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <p class="text-sm text-gray-600 max-w-md text-center mt-2 mb-5">
                        Get all 3 Optimized Bio Drafts, your Best Performing Content Format, and
                        the Pinned Tweet ready to copy-paste.
                      </p>
                      <button
                        type="button"
            ...
            <div class="tab-content p-6 sm:p-8">
              ...
              >
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-500">
                    Suggested pricing, packages &amp; plug-and-play templates to pitch
                    sponsors.
                  </p>
                </div>
                {#if !user?.isPro}
                  ...
                  <div class="space-y-6">
                    <div class="border border-green-200 rounded-lg p-4 bg-green-50/70 mb-6">
                        ...
                        <p class="mt-2 text-base font-bold text-gray-900">
                            ...
                            <span class="text-green-600 font-normal text-sm">
                                (Calculated based on {analysisData.analysis.avgEngagementRate}% Avg. ER &amp; follower count)
                            </span>
                        </p>
                    </div>
                      ...
                      <h3 class="text-base font-semibold text-gray-900 mb-3">Suggested Sponsorship Packages</h3>
                        <div class="p-3 border border-gray-200 rounded-lg bg-white space-y-2">
                           {#each displayMonetizationData.packages as pkg}
                              <div class="flex justify-between items-center border-b border-gray-100 last:border-b-0 py-1.5">
                                  <div class="text-sm text-gray-700">
                                      <strong class="text-primary">{pkg.name}:</strong> {pkg.description}
                                  </div>
                                  <span class="text-sm font-bold text-green-600">{pkg.price}</span>
                              </div>
                           {/each}
                        </div>
                    </div>
                    ...
                    <div>
                      <h3 class="text-base font-semibold text-gray-900 mb-3">Pitch Email Snippet (Ready to Copy)</h3>
                        <div class="p-4 border border-gray-200 rounded-xl bg-white shadow-soft">
                            <p class="font-bold text-primary mb-2">The "Synergy First" Pitch</p>
                            <p class="italic text-sm text-gray-800">
                                {displayMonetizationData.pitchEmailSnippet}
                            </p>
                            <button
                                on:click={() => { navigator.clipboard.writeText(displayMonetizationData.pitchEmailSnippet); addToast('Snippet Copied!', 'success'); }}
                                class="mt-3 inline-flex items-center rounded-lg bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-200 transition-colors"
                            >
                                Copy Snippet
                            </button>
                  ...
                  <div class="relative">
                    <div class="border border-green-200 rounded-lg p-4 bg-green-50/70 mb-6">
                        ...
                        <p class="mt-2 text-base font-bold text-gray-900">
                            $325 USD
                            <span class="text-green-600 font-normal text-sm">
                                (Calculated based on 1.9% Avg. ER &amp; 50K Impressions/mo)
                            </span>
                        </p>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="p-3 border border-gray-200 rounded-lg bg-white">
                          <p class="font-bold text-primary">Sponsorship Packages</p>
                            <ul class="list-disc pl-4 space-y-1">
                              ...
                              <li>**Premium:** 3 tweets over 1 week, included in a Thread ($850)</li>
                              <li>
                                **Acquisition:** 1 Month Partnership, dedicated Thread, Bio mention
                                ($2,500)
                              </li>
                          </ul>
                        </div>
                        <div class="p-3 border border-gray-200 rounded-lg bg-white">
                          <p class="font-bold text-primary">Pitch Email Template Snippet</p>
                          <p class="italic">
                            "Hi [Brand Contact], I saw your recent launch and noticed a huge
                            synergy with my audience (SaaS Founders, 1.9% ER). I have a 3-tier
                            package perfect for driving MQLs..."
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        ...
                        <p class="text-sm text-gray-600 max-w-md text-center mt-2 mb-5">
                          Access suggested pricing, packages, and ready-to-use email &amp; DM
                          templates to pitch brands and sponsors.
                        </p>
                        <button
                          type="button"
                          class="rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-soft hover:bg-gray-900 transition-colors"
