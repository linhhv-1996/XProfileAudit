  ...
  <div class="flex items-center gap-3 sm:gap-4 w-full sm:w-auto min-w-0">
    ...
    <div class="flex-1 min-w-0">
      <div class="flex flex-wrap items-baseline gap-x-2 mb-1">
        ...
        <a href={`https://twitter.com/${profile?.handle}`} target="_blank" rel="noreferrer" class="text-sm text-gray-500 hover:text-gray-700 truncate">@{profile?.handle}</a>
      </div>
      
      <div class="flex items-center gap-3 text-xs sm:text-sm text-gray-500">
        <div class="flex items-center gap-1" title="Followers">
            <span class="font-bold text-gray-800">{formatNumber(profile?.followers || 0)}</span>
        ...
        <div class="flex items-center gap-1" title="Following">
            <span class="font-bold text-gray-800">{formatNumber(profile?.following || 0)}</span>
        ...
        <div class="flex items-center gap-1" title="Tweets">
            <span class="font-bold text-gray-800">{formatNumber(profile?.tweetsCount || 0)}</span>
